...
fclose(fileID);
M1=single(M1');% ======= why does it need to transpose matrix??? ========
...
fclose(fileID2);
M2=single(M2'); % ======= why does it need to transpose matrix??? ========
...
%% 4. Exp
expectedExp = single(expm(double(M1)));  % === function 'expm' ===
dlmwrite([output_path,'/expectedExpMatrix.txt'],expectedExp,'precision','%.6f','delimiter',' ');
%% 5. Log
expectedLog = single(logm(double(M1)));  % === function 'logm' ===
