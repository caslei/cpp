...
for i=1:2
            ...
            if (i+1)==2
                ...
                for a=1:3
                    for b=1:3
                        ...
                        jacobian(2,1)=jacobian(2,1) + first(a) * basis(b) * grid_data(x+a-2, y+b-2, 1, 1, 2);
                        jacobian(2,2)=jacobian(2,2) + basis(a) * first(b) * grid_data(x+a-2, y+b-2, 1, 1, 2);
                    end
                end
                ...
                for a=1:2
                    for b=1:2
                        constraint_approx = constraint_approx + (0.5*(jacobian(a,b)+jacobian(b,a)))^2;
                    end
                end
            else
                for z=2:grid_dim(3)-1
                    ...
                    for a=1:3
                        for b=1:3
                            for c=1:3
                                jacobian(1,1)=jacobian(1,1) + first(a) * basis(b) * basis(c) * grid_data(x+a-2, y+b-2, z+c-2, 1, 1);
                                jacobian(1,2)=jacobian(1,2) + basis(a) * first(b) * basis(c) * grid_data(x+a-2, y+b-2, z+c-2, 1, 1);
                                jacobian(1,3)=jacobian(1,3) + basis(a) * basis(b) * first(c) * grid_data(x+a-2, y+b-2, z+c-2, 1, 1);
                                
                                jacobian(2,1)=jacobian(2,1) + first(a) * basis(b) * basis(c) * grid_data(x+a-2, y+b-2, z+c-2, 1, 2);
                                jacobian(2,2)=jacobian(2,2) + basis(a) * first(b) * basis(c) * grid_data(x+a-2, y+b-2, z+c-2, 1, 2);
                                jacobian(2,3)=jacobian(2,3) + basis(a) * basis(b) * first(c) * grid_data(x+a-2, y+b-2, z+c-2, 1, 2);
                                
                                jacobian(3,1)=jacobian(3,1) + first(a) * basis(b) * basis(c) * grid_data(x+a-2, y+b-2, z+c-2, 1, 3);
                                jacobian(3,2)=jacobian(3,2) + basis(a) * first(b) * basis(c) * grid_data(x+a-2, y+b-2, z+c-2, 1, 3);
                                jacobian(3,3)=jacobian(3,3) + basis(a) * basis(b) * first(c) * grid_data(x+a-2, y+b-2, z+c-2, 1, 3);
                            end
                        end
                    end
    ...
    def_dim=[def_image.hdr.dime.dim(2), def_image.hdr.dime.dim(3), def_image.hdr.dime.dim(4)];
    ...
    for x=0:def_dim(1)-1
        ...
        for y=0:def_dim(2)-1
            ...
            if (i+1)==2
                ...
                for a=1:4
                    for b=1:4
                        jacobian(1,1)=jacobian(1,1) + first_x(a) * basis_y(b) * grid_data(pre_x+a, pre_y+b, 1, 1, 1);
                        jacobian(1,2)=jacobian(1,2) + basis_x(a) * first_y(b) * grid_data(pre_x+a, pre_y+b, 1, 1, 1);
                        jacobian(2,1)=jacobian(2,1) + first_x(a) * basis_y(b) * grid_data(pre_x+a, pre_y+b, 1, 1, 2);
                        jacobian(2,2)=jacobian(2,2) + basis_x(a) * first_y(b) * grid_data(pre_x+a, pre_y+b, 1, 1, 2);
                    end
                end
                jacobian = orientation(1:2,1:2) * jacobian';
                rotation = polarDecomposition(jacobian);
                ...
                for z=0:def_dim(3)-1
                    ...
                    for a=1:4
                        for b=1:4
                            for c=1:4
                                jacobian(1,1)=jacobian(1,1) + first_x(a) * basis_y(b) * basis_z(c) * grid_data(pre_x+a, pre_y+b, pre_z+c, 1, 1);
                                jacobian(1,2)=jacobian(1,2) + basis_x(a) * first_y(b) * basis_z(c) * grid_data(pre_x+a, pre_y+b, pre_z+c, 1, 1);
                                jacobian(1,3)=jacobian(1,3) + basis_x(a) * basis_y(b) * first_z(c) * grid_data(pre_x+a, pre_y+b, pre_z+c, 1, 1);
                                
                                jacobian(2,1)=jacobian(2,1) + first_x(a) * basis_y(b) * basis_z(c) * grid_data(pre_x+a, pre_y+b, pre_z+c, 1, 2);
                                jacobian(2,2)=jacobian(2,2) + basis_x(a) * first_y(b) * basis_z(c) * grid_data(pre_x+a, pre_y+b, pre_z+c, 1, 2);
                                jacobian(2,3)=jacobian(2,3) + basis_x(a) * basis_y(b) * first_z(c) * grid_data(pre_x+a, pre_y+b, pre_z+c, 1, 2);
                                
                                jacobian(3,1)=jacobian(3,1) + first_x(a) * basis_y(b) * basis_z(c) * grid_data(pre_x+a, pre_y+b, pre_z+c, 1, 3);
                                jacobian(3,2)=jacobian(3,2) + basis_x(a) * first_y(b) * basis_z(c) * grid_data(pre_x+a, pre_y+b, pre_z+c, 1, 3);
                                jacobian(3,3)=jacobian(3,3) + basis_x(a) * basis_y(b) * first_z(c) * grid_data(pre_x+a, pre_y+b, pre_z+c, 1, 3);
                            end
                        end
                    end
                    jacobian = orientation * jacobian';
